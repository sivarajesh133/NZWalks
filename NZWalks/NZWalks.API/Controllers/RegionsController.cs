...
namespace NZWalks.API.Controllers
{
    ...
    public class RegionsController : Controller
    {
        ...
        private  bool ValidateAddRegionAsync(Models.DTO.AddRegionRequest addRegionRequest)
        {
            ...
            {
                ModelState.AddModelError(nameof(addRegionRequest), $" add request cannot be empty");
                return false;
            }

            if(string.IsNullOrWhiteSpace(addRegionRequest.Code))
            {
                ModelState.AddModelError(nameof(addRegionRequest.Code), $"{nameof(addRegionRequest.Code)} cannot be null or whitespace or empty ");

            }
            if (string.IsNullOrWhiteSpace(addRegionRequest.Name))
            {
                ModelState.AddModelError(nameof(addRegionRequest.Name), $"{nameof(addRegionRequest.Name)} cannot be null or whitespace or empty ");
        ...
        private bool ValidateUpdateAsync(Models.DTO.UpdateRegionRequest updateRegionRequest)
        {
            ...
            {
                ModelState.AddModelError(nameof(updateRegionRequest), $" add request cannot be empty");
                return false;
            }

            if (string.IsNullOrWhiteSpace(updateRegionRequest.Code))
            {
                ModelState.AddModelError(nameof(updateRegionRequest.Code), $"{nameof(updateRegionRequest.Code)} cannot be null or whitespace or empty ");

            }
            if (string.IsNullOrWhiteSpace(updateRegionRequest.Name))
            {
                ModelState.AddModelError(nameof(updateRegionRequest.Name), $"{nameof(updateRegionRequest.Name)} cannot be null or whitespace or empty ");
